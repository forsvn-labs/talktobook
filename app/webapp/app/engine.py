...
def render_cover(
    ...
    *,
    ...
    Parameterized version of the CLI's Pillow cover so a custom accent can be
    used. Returns False (no cover) if Pillow is unavailable.
    """
    if not HAS_PILLOW:
        return False
...
def cover_prompt(title: str, author: str | None) -> str:
    """AI image-gen prompt for a custom cover (delegates to the engine)."""
