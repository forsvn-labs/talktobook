...
content), built with the same engine (no watermark, designed cover) so visitors
can download and read a real output before trying the tool.
...
def _build_one(s: dict, dest_root: Path) -> dict:
    ...
    if not epub.exists() or stale:
        ...
        engine.generate(
            ...
            title=s["title"], author=s["author"], source_url=None,
